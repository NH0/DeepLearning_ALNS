...
def parse_epoch_line(line, search, start_epoch, display_every_n_epoch, has_display_every_n_epoch, loss, test_loss):
    ...
    return start_epoch, display_every_n_epoch, has_display_every_n_epoch


def parse_result_gatedgcn(result_file_path):
    loss = []
    test_loss = []
    ...
    with open(result_file_path, 'r') as result_file:
        for line in result_file.readlines():
            if line.startswith("# Hidden dimensions : "):
                hidden_dimensions = re.search(r"\[(\d+, )*\d+\]$", line).group()
            elif line.startswith("# Number of convolutions layers : "):
                convolution_layers = re.search(r"\[(\d+, )*\d+\]$", line).group()
            elif line.startswith("# Number of linear layers : "):
                linear_layers = re.search(r"(\[(\d+, )*\d+\]|\d+)$", line).group()
            elif line.startswith("Epoch"):
                search = re.search(r"Epoch (\d+),", line)
                if search is not None:
                    start_epoch, display_every_n_epoch, has_display_every_n_epoch = \
                        parse_epoch_line(line,
                                         search,
                                         display_every_n_epoch,
                                         has_display_every_n_epoch,
                                         start_epoch,
                                         loss,
                                         test_loss)
...
def display_loss(result_file, network='gatedgcn'):
    result_file_path = get_project_root_path() + '/results/' + result_file
    if network == 'gatedgcn':
        result = parse_result_gatedgcn(result_file_path)
    else:
    ...
    if network == 'gatedgcn':
        plt.text(0.98, 0.97,
                 'Hidden convolution dimensions : ' + result['hidden_dimensions']
                 + '\nNumber of conv layers : ' + result['convolution_layers']
                 + '\nNumber of lin layers : ' + result['linear_layers'],
...
def main(loss_file):
    display_loss(loss_file, network='gcn')
